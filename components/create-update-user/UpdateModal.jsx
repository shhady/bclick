...
export default function UpdateModal({ formData, setFormData, onSubmit, isOpen, setIsOpen }) {
  ...
  const { setGlobalUser, globalUser } = useUserContext();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Make the API call directly instead of using onSubmit
      ...
      if (response.ok) {
        ...
        if (result && result._id) {
          ...
          console.error('Invalid user data received from API:', result);
        }
      } else {
